 ...
 *   - クエリ埋め込み: text-embedding-004（RAGの検索クエリをベクトル化）
 ...
 *       EMBED_MODEL    = text-embedding-004（任意。未設定なら既定値）
...
function embed(body, key){
  const model = PROP('EMBED_MODEL') || 'text-embedding-004';
  ...
  const payload = { model: 'models/' + model, content: { parts: [{ text: text }] } };
